...
Mdarray::Mdarray(const Storage &storage,
                                               const Shape &shape,
                                               const IndexArray &stride,
                                               bool requires_grad)
    : Exp<MdarrayImpl>(
          Allocator::UniqueConstruct<MdarrayImpl>(
...
Mdarray::Mdarray(const BasicData *data,
                                               const Shape &shape,
                                               bool requires_grad)
    : Exp<MdarrayImpl>(
          Allocator::UniqueConstruct<MdarrayImpl>(
...
Mdarray::Mdarray(const Shape &shape,
                                               bool requires_grad)
    : Exp<MdarrayImpl>(
          Allocator::UniqueConstruct<MdarrayImpl>(
              shape, requires_grad)) {}
...
Mdarray::Mdarray(
    Allocator::UniquePtr<MdarrayImpl> &&ptr)
    : Exp<MdarrayImpl>(std::move(ptr)) {}

Mdarray &Mdarray::operator=(
    const Mdarray &other) {
  impl_ptr_->operator=(other.Impl());
  return *this;
}

Index Mdarray::DimensionsSize() const {
  return impl_ptr_->DimensionsSize();
}

Index Mdarray::Size(Index idx) const {
...
Mdarray Mdarray::Slice(Index dim,
                                                     Index idx) const {
  return Mdarray{impl_ptr_->Slice(dim, idx)};
}

Mdarray Mdarray::Slice(Index dim, Index start_idx,
                                                     Index end_idx) const {
  return Mdarray{impl_ptr_->Slice(dim, start_idx, end_idx)};
}

Mdarray Mdarray::Transpose(Index dim1,
                                                         Index dim2) const {
  return Mdarray{impl_ptr_->Transpose(dim1, dim2)};
}

Mdarray Mdarray::Permute(
...
Mdarray Mdarray::Squeeze() const {
  return Mdarray{impl_ptr_->Squeeze()};
}
...
Mdarray Mdarray::Grad() const {
  return Mdarray{impl_ptr_->Grad()};
}
